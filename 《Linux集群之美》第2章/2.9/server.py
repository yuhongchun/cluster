...
class WSSHBridge(object):
    ...
    def open(self, hostname, port=22, username=None, password=None,
        ...
        try:
            ...
            if private_key:
                ...
                pkey = paramiko.RSAKey.from_private_key_file(private_key,key_passphrase)
            self._ssh.connect(
                hostname=hostname,
                port=port,
                username=username,
    ...
    def execute(self, command, term='xterm'):
         """ Execute a command aboout Docker CLI on the remote server

         """
         transport = self._ssh.get_transport()
         channel = transport.open_session()
         channel.get_pty(term)
         #command = "docker exec -ti {} /bin/bash".format(container_id)
         #print command
